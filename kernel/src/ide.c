...
#define ATA_IRQ_OFF		0x2
#define ATA_IRQ_ON		0x0
...
#define ATA_BMCMD_START	0x1
#define ATA_BMCMD_READ	0x0
#define ATA_BMCMD_WRITE	0x8

#define ATA_BMSTATUS_ACTIVE	0x1
#define ATA_BMSTATUS_ERROR	0x2
#define ATA_BMSTATUS_INTERRUPT	0x4
...
static void handle_irq_primary(uint32_t UNUSED(error)) {
   ...
   if(status & ATA_BMSTATUS_INTERRUPT) {
      ...
      panic("IDE - Primary Channel IRQ Confict");
   }
}
...
static void handle_irq_secondary(uint32_t UNUSED(error)) {
   ...
   if(status & ATA_BMSTATUS_INTERRUPT) {
      ...
      panic("IDE - Secondary Channel IRQ Confict");
   }
}
...
static int32_t ide_ata_access(bool write, bool same, uint8_t drive, uint64_t numsects, uint32_t lba, void * edi) {
   ...
   if (lba_mode > 0 /* There is no CHS (lba_mode == 0) for DMA */ && ide_devices[drive].features & ATA_FEATURE_DMA) { 
      ...
      uint64_t bytes = sector_size * numsects;
      uint32_t i;
      ...
      *(channels[channel].prdt + i) |= 0x8000000000000000; // mark last PRD entry as EOT
   } else {
